<template>
    <div class="flex">
        <div id="Header" class="fixed w-[420px] z-10">
            ...
            <div class="bg-[#202c33] w-full flex justify-between items-center px-3 py-2">
                <img class="rounded-full ml-1 w-10" :src="userStore.picture || ''" alt="">
                <div class="flex items-center justify-center">
                    <AccountGroupIcon fillColor="#aebac1" class="rounded-full cursor-pointer w-auto p-2 mr-6 hover:bg-[#ffffff1a]" />
                    <DotsVerticalIcon @click="logout" fillColor="#aebac1" class="rounded-full cursor-pointer w-auto p-2 hover:bg-[#ffffff1a]" />
                </div>
            </div>

            <div id="Search" class=" w-full px-2 border-b border-[#2d3c45b5] shadow-sm">
                <div class="px-1 m-2 bg-[#202c33] flex items-center justify-center rounded-md">
                    <MagnifyIcon fillColor="#aebac1" :size="18" class="ml-2" />
                    <input 
                    @click="showFindFriends = !showFindFriends"
                    class="ml-5 apperance-none bg-[#202c33] placeholder:text-gray-500 text-gray-400 focus:outline-none py-2 text-sm px-10 w-full rounded-lg"
                    autocomplete="off"
                    type="text"
        ...
        <div v-if="showFindFriends">
            <FindFriendsView  class="pt-28 bg-[#111b21]" />
        </div>
        <div v-else>
            <ChatsView class="bg-[#111b21] mt-[100px]" />
        </div>
